...
const RegionSelector = ({ onRegionChange, selectedRegions = [] }) => {
    ...
    const fetchDistricts = async (provinceName) => {
        setLoading(true);
        try {
            const response = await axios.get(`${API_BASE_URL}/provinces/${encodeURIComponent(provinceName)}/districts`);
    ...
    const fetchDistrictsForGyeonggi = async () => {
        setLoading(true);
        try {
            const response = await axios.get(`${API_BASE_URL}/provinces/${encodeURIComponent('경기도')}/districts`);
