...
class HogangCrawler:
    def __init__(self):
        self.base_url = "https://www.hogangnono.com"
        self.ua = UserAgent()
    ...
    def crawl(self, regions=None):
        ...
        for region in regions:
            try:
                ...
                print(f"호갱노노 크롤링 오류 ({region}): {str(e)}")
                continue
    ...
    def crawl_region(self, region_name):
        ...
        try:
            ...
            # 지역별 매핑 (실제 호갱노노 지역 코드에 맞게 수정 필요)
            region_mapping = {
                '서울특별시': 'seoul',
                '경기도': 'gyeonggi',
                '부산광역시': 'busan',
            ...
            # 아파트 목록 추출 (실제 호갱노노 HTML 구조에 맞게 수정 필요)
    ...
    def extract_apartments(self, soup, region_name):
        ...
        try:
            # 실제 호갱노노 HTML 구조에 맞게 수정 필요
            # 현재는 예시 데이터로 대체
            apartment_elements = soup.find_all('div', class_='property-card')
            
            for element in apartment_elements[:10]:  # 상위 10개만
                try:
                    # 아파트명 추출
                    name_elem = element.find('h4', class_='complex-name')
                    if not name_elem:
                        continue
                    ...
                    # 가격 정보 추출
                    price_elem = element.find('span', class_='price-value')
                    avg_price = 0
                    if price_elem:
                    ...
                    price_change_rate = random.uniform(-3.5, 3.5)
            ...
            if not transactions:
                transactions = self.generate_sample_data(region_name, 'hogang')
    ...
    def generate_sample_data(self, region_name, source):
        """예시 거래 데이터 생성"""
        sample_apartments = [
            f"{region_name}호갱아파트1단지",
            f"{region_name}호갱아파트2단지",
            f"{region_name}호갱아파트3단지",
            f"{region_name}호갱아파트4단지",
            f"{region_name}호갱아파트5단지"
        ]
        ...
        for i, complex_name in enumerate(sample_apartments):
            transactions.append({
                ...
                'avg_price': random.randint(320000000, 780000000),
                'source': source
            })
    ...
    def generate_sample_price_data(self, region_name):
        ...
        return [{
            'date': datetime.now().strftime('%Y-%m-%d'),
            'region_name': region_name,
            'avg_price': random.randint(420000000, 680000000),
